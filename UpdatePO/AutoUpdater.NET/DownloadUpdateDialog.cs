...
namespace AutoUpdaterDotNET
{
    internal partial class DownloadUpdateDialog : Form
    {
        ...
        public DownloadUpdateDialog(string downloadURL)
        {
            ...
            this.Location = new Point(Screen.PrimaryScreen.WorkingArea.Width - Width,
        ...
        private void OnDownloadProgressChanged(object sender, DownloadProgressChangedEventArgs e)
        {
            ...
            else
            {
                ...
                if (totalSeconds > 0)
                {
                    ...
                    labelInformation.Text = $"Загружается обновление... {BytesToString(bytesPerSecond)}";
                }
            }
            this.Text = $"{BytesToString(e.BytesReceived)} / {BytesToString(e.TotalBytesToReceive)}";
        ...
        private static string TryToFindFileName(string contentDisposition, string lookForFileName)
        {
            ...
            if (!string.IsNullOrEmpty(contentDisposition))
            {
                ...
                if (fileName.StartsWith("\""))
                {
                    ...
                    int i = file.IndexOf("\"", StringComparison.CurrentCultureIgnoreCase);
                    if (i != -1)
                    {
